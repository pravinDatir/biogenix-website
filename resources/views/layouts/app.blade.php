...
<head>
    ...
    <meta name="description" content="@yield('meta_description', 'Biogenix Healthcare Solutions — Precision diagnostics, innovative life science research tools, and medical instruments for laboratories and healthcare professionals.')">
        ...
        @unless ($suppressShellAlerts)
            <div class="container space-y-4">
                @if (session()->has('impersonation.impersonator_id'))
                    <div class="animate-entrance rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-800">
                        You are currently impersonating another user.
                        <form method="POST" action="{{ route('impersonation.stop') }}" class="inline-block">
                            @csrf
                            <button type="submit" class="ml-2 inline-flex h-9 items-center justify-center rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Stop Impersonation</button>
                        </form>
                    </div>
                @endif
...
{{-- Floating Support Widget --}}<div class="support-widget">
    <div id="supportTicketForm" class="support-panel is-hidden hidden">
        <div class="mb-5 flex items-center justify-between">
            ...
            <button onclick="toggleSupportForm()" class="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
        ...
        <form action="{{ route('contact') }}" method="GET" class="space-y-4 text-left">
            <div>
                <label for="supportName" class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Full Name</label>
                <input type="text" id="supportName" name="name" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-primary-500 focus:bg-white focus:ring-4 focus:ring-primary-500/10" placeholder="John Doe" required>
            </div>
            <div>
                <label for="supportEmail" class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Email Address</label>
                <input type="email" id="supportEmail" name="email" class="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-primary-500 focus:bg-white focus:ring-4 focus:ring-primary-500/10" placeholder="you@example.com" required>
            </div>
            <div>
                <label for="supportMessage" class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">Message</label>
                <textarea id="supportMessage" name="message" rows="3" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-primary-500 focus:bg-white focus:ring-4 focus:ring-primary-500/10" placeholder="How can we help you today?" required></textarea>
            </div>
            <button type="submit" class="inline-flex h-12 w-full items-center justify-center rounded-xl bg-primary-600 px-5 text-sm font-bold text-white shadow-lg shadow-primary-600/20 transition hover:bg-primary-700 active:scale-[0.98]">
                Send Message
            </button>
        </form>
    </div>
...
<button id="scrollTopBtn" class="scroll-top-btn is-hidden" aria-label="Scroll to top" title="Scroll to top">
