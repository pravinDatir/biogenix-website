<aside class="hidden lg:block w-64 flex-shrink-0">
    <nav class="sticky top-24 space-y-1.5 bg-white p-4 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100 pb-6">
        ...
        $navLinks = [
            ...
            ['icon' => 'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z', 'title' => 'Order Management', 'route' => 'adminPanel.orders'],
        ...
        @foreach($navLinks as $link)
            ...
            <a href="{{ isset($link['route']) ? route($link['route']) : '#' }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl font-semibold text-[13px] transition w-full relative {{ $isActive ? 'bg-[#091b3f] text-white font-bold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800' }}">
                ...
                @endif
                <span class="leading-tight z-10">{{ $link['title'] }}</span>
            </a>
        @endforeach
    </nav>
</aside>
