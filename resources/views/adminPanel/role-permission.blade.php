...
@section('admin_content')
    @php
        ...
        $subAdmins = [
            ...
            ['name' => 'Elena Rodriguez', 'scope' => 'Scope: Inventory Module Only', 'badge' => 'Module-Based', 'avatar' => 'ER', 'tone' => 'bg-amber-100 text-amber-700'],
        ];
    @endphp

    <div class="space-y-6 text-slate-900">
        <div class="space-y-2">
            <h1 class="font-display text-[32px] font-extrabold tracking-[-0.04em] text-slate-950 sm:text-[38px]">Roles &amp; Permissions Center</h1>
            <p class="max-w-3xl text-[15px] text-slate-500">Manage global access controls, granular functional mapping, and individual user overrides.</p>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,2.3fr)_minmax(18rem,1fr)]">
            <section class="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
                <div class="flex items-center justify-between gap-4 border-b border-slate-100 px-6 py-5">
                    <div class="flex items-center gap-3">
                        <svg class="h-[18px] w-[18px] text-[#102f72]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        <h2 class="text-[15px] font-extrabold text-slate-950">System Roles</h2>
                    </div>

                    <button type="button" class="inline-flex items-center justify-center gap-2 rounded-xl bg-[#0b255e] px-4 py-3 text-[13px] font-extrabold text-white shadow-[0_10px_20px_rgba(11,37,94,0.18)] transition hover:brightness-105">
                            ...
                            @foreach ($systemRoles as $role)
                                <tr class="border-t border-slate-100">
                                    ...
                                    <td class="px-4 py-5 text-center">
                                        <span class="inline-flex rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.12em] text-emerald-600">{{ $role['status'] }}</span>
                                    </td>
                                    <td class="px-6 py-5 text-right">
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-50 hover:text-[#102f72]" aria-label="Edit {{ $role['name'] }}">
            ...
            <section class="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
                <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-5">
                    ...
                    <h2 class="text-[15px] font-extrabold text-slate-950">Permissions</h2>
                </div>

                <div class="space-y-3 p-4 sm:p-5">
                    @foreach ($permissions as $permission)
                        <button type="button" class="flex w-full items-start justify-between gap-3 rounded-2xl border border-slate-100 bg-white px-4 py-4 text-left transition hover:border-slate-200 hover:shadow-[0_8px_18px_rgba(15,23,42,0.04)]">
                            <div class="min-w-0">
                    ...
                    <button type="button" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#0b255e] px-4 py-3 text-[13px] font-extrabold text-white shadow-[0_10px_20px_rgba(11,37,94,0.18)] transition hover:brightness-105">
                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                        </svg>
                        Add Permission
        ...
        <section class="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
            <div class="flex flex-col gap-3 border-b border-slate-100 px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-[15px] font-extrabold text-slate-950">Permission Mapping Matrix</h2>
                </div>

                <div class="flex items-center gap-3 text-[12px] text-slate-500">
                    <span class="inline-flex h-2.5 w-2.5 rounded-full bg-[#12367d] shadow-[0_0_0_4px_rgba(18,54,125,0.06)]"></span>
                    ...
                    <thead>
                        <tr class="bg-slate-50/80">
                            ...
                            @foreach ($matrixRoles as $roleIndex => $role)
                                <th colspan="{{ count($role['actions']) }}" class="border-b border-slate-200 px-4 py-4 text-center text-[13px] font-extrabold uppercase tracking-[0.1em] text-[#21325b] {{ $roleIndex < count($matrixRoles) - 1 ? 'border-r' : '' }}">
                                    {{ $role['label'] }}
                                </th>
                            @endforeach
                        </tr>
                                ...
                                @foreach ($matrixRoles as $roleIndex => $role)
                                    @foreach ($row[$role['key']] as $actionIndex => $enabled)
                                        <td class="border-b border-slate-100 px-2 py-5 text-center {{ $actionIndex === count($row[$role['key']]) - 1 && $roleIndex < count($matrixRoles) - 1 ? 'border-r' : '' }}">
                                            <span class="mx-auto inline-flex h-[18px] w-[18px] items-center justify-center rounded-full border-[1.6px] {{ $enabled ? 'border-[#12367d] bg-[#12367d] text-white' : 'border-slate-300 bg-white text-transparent' }}">
                                                @if ($enabled)
            ...
            <div class="flex justify-end px-4 py-4 sm:px-6">
                <button type="button" class="inline-flex items-center justify-center rounded-xl bg-[#0b255e] px-7 py-3 text-[14px] font-extrabold text-white shadow-[0_10px_20px_rgba(11,37,94,0.18)] transition hover:brightness-105">
                    Save Matrix Changes
                </button>
            </div>
        </section>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
            <section class="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
                <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-5">
                    ...
                    <h2 class="text-[15px] font-extrabold text-slate-950">User-Based Overrides</h2>
                </div>
                ...
                <div class="space-y-5 p-6">
                    ...
                    <div class="flex flex-col gap-4 border-b border-dashed border-slate-200 pb-5 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="inline-flex rounded-full bg-blue-50 px-2 py-1 text-[9px] font-extrabold uppercase tracking-[0.12em] text-blue-600">1 Add</span>
                    ...
                    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <button type="button" class="inline-flex items-center gap-1 text-[12px] font-extrabold uppercase tracking-[0.12em] text-emerald-600">
                            <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                            </svg>
                            Add
            ...
            <section class="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
                <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-5">
                    ...
                    <h2 class="text-[15px] font-extrabold text-slate-950">Group &amp; Department Access</h2>
                </div>
                ...
                <div class="space-y-4 p-6">
                    @foreach ($departments as $department)
                        <div class="flex items-center justify-between gap-4 rounded-2xl border border-slate-100 px-4 py-4">
                            ...
                            <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-50 hover:text-slate-600">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <button type="button" class="flex min-h-[52px] w-full items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white text-[15px] font-semibold text-slate-500 transition hover:border-slate-400 hover:text-slate-700">
        ...
        <section class="overflow-hidden rounded-[24px] border border-slate-200/80 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
            <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-5">
                ...
                <h2 class="text-[15px] font-extrabold text-slate-950">Delegation &amp; Impersonation</h2>
            </div>
            ...
            <div class="grid xl:grid-cols-2">
                <div class="space-y-5 p-6 xl:border-r xl:border-slate-100">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <button type="button" class="inline-flex items-center gap-2 text-[13px] font-bold text-[#102f72]">
                ...
                <div class="space-y-5 p-6">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <label class="inline-flex items-center gap-2 text-[12px] font-bold text-slate-700">
                            <input type="checkbox" checked class="h-4 w-4 rounded border-slate-300 text-[#12367d] focus:ring-[#12367d]">
                    ...
                    <div class="space-y-3">
                        <div class="flex flex-col gap-1 text-[13px] sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <span class="text-slate-700">Admin logged in as <a href="#" class="font-bold text-[#102f72] underline">r.taylor@biogenix.com</a></span>
                        </div>
                        <div class="flex flex-col gap-1 text-[13px] sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-slate-500">Yesterday, 04:45 PM</span>
                            <span class="text-slate-700">Admin logged in as <a href="#" class="font-bold text-[#102f72] underline">k.mills@biogenix.com</a></span>
                        </div>
                    </div>

                    <button type="button" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#0b255e] px-4 py-3 text-[14px] font-extrabold text-white shadow-[0_10px_20px_rgba(11,37,94,0.18)] transition hover:brightness-105">
