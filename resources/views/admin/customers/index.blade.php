...
@section('admin_content')
    ...
    <div class="mb-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex items-center gap-3">
            <div class="relative w-64">
                ...
                <input id="global-customer-search" type="text" placeholder="Global search..." class="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-4 text-sm font-medium text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-primary-600 focus:bg-white focus:ring-1 focus:ring-primary-600">
            </div>
        </div>
    </div>
    ...
    <div id="pending-verifications-section" class="mb-5 rounded-2xl border border-slate-200/60 bg-slate-100 px-6 py-4">
        <div class="mb-3 flex items-center justify-between">
            <div class="flex items-center gap-2.5">
                <svg class="h-5 w-5 shrink-0 text-secondary-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
                </svg>
                <span id="pending-count-label" class="text-sm font-bold text-amber-800">Pending User Verifications ({{ count($pendingApplicants) }})</span>
            </div>
            <button type="button" class="text-[12px] font-bold text-secondary-700 transition hover:text-amber-900 cursor-pointer">View All Pending</button>
        </div>

        <div id="pending-list" class="space-y-2.5">
            @foreach($pendingApplicants as $applicant)
                <div class="flex items-center justify-between gap-4 rounded-xl border border-slate-100 bg-white px-4 py-3 shadow-sm" data-pending-id="{{ $applicant['id'] }}" data-applicant-name="{{ $applicant['name'] }}" data-application-category="{{ $applicant['category'] }}">
                    <div class="flex min-w-0 items-center gap-3">
                        <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full {{ $applicant['color'] }} text-[11px] font-black text-white">{{ $applicant['initials'] }}</div>
                        <div class="min-w-0">
                            <p class="truncate text-[13px] font-bold text-slate-900">{{ $applicant['name'] }}</p>
                            <p class="truncate text-[11px] font-medium text-slate-500">{{ $applicant['email'] }} &bull; {{ $applicant['applicationLabel'] }}</p>
                        </div>
                    </div>
                    <div class="pending-action-group flex shrink-0 items-center gap-2">
                        <button type="button" onclick="handleVerification(this, 'approve', {{ $applicant['id'] }})" class="rounded-lg bg-primary-600 px-4 py-1.5 text-[12px] font-bold text-white transition hover:bg-primary-700 cursor-pointer">Approve</button>
                        <button type="button" onclick="handleVerification(this, 'reject', {{ $applicant['id'] }})" class="rounded-lg border border-rose-200 bg-white px-4 py-1.5 text-[12px] font-bold text-rose-600 transition hover:bg-rose-50 cursor-pointer">Reject</button>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="mb-5 rounded-2xl border border-slate-100 bg-white p-5 shadow-[var(--ui-shadow-soft)]">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="mt-1 text-[13px] text-slate-500">Use the type-ahead dropdown to jump directly to a B2B customer profile.</p>
            </div>
        ...
        <div id="b2b-client-typeahead" class="relative mt-4">
            <div class="relative">
                ...
                <input id="b2b-client-search" type="text" autocomplete="off" placeholder="Search B2B company by name or business email..." class="w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-10 pr-4 text-sm font-medium text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-primary-600 focus:bg-white focus:ring-1 focus:ring-primary-600">
            </div>

            <input id="b2b-client-selected-url" type="hidden" value="">

            <div id="b2b-client-suggestions" class="absolute inset-x-0 top-full z-20 mt-2 hidden max-h-72 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
                @foreach($b2bCustomers as $customer)
                    ...
                    @endphp
                    <button type="button" class="b2b-client-option flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left transition hover:bg-slate-50 cursor-pointer" data-name="{{ strtolower($customer['name']) }}" data-email="{{ strtolower($customer['email']) }}" data-url="{{ $detailsUrl }}">
                        <div class="min-w-0">
                            <p class="text-sm font-bold text-slate-900">{{ $customer['name'] }}</p>
                            <p class="text-xs font-medium text-slate-500">{{ $customer['email'] }}</p>
                        </div>
                        <span class="inline-flex shrink-0 items-center rounded-full border border-primary-200/60 bg-primary-50 px-2.5 py-1 text-[10px] font-bold text-primary-700">B2B</span>
                    </button>
                @endforeach

                <div id="b2b-client-empty-state" class="hidden px-3 py-4 text-sm font-medium text-slate-400">No matching B2B clients found.</div>
            </div>
        </div>

        <div class="mt-4 flex justify-end">
            <button id="view-selected-b2b-client" type="button" class="inline-flex items-center gap-2 rounded-xl bg-primary-600 px-4 py-2.5 text-sm font-bold text-white shadow-md shadow-primary-600/20 transition hover:bg-primary-700 cursor-pointer disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500 disabled:shadow-none" disabled>
    ...
    <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[var(--ui-shadow-soft)]">
        <div class="flex flex-col justify-between gap-3 border-b border-slate-100 px-6 py-4 sm:flex-row sm:items-center">
            ...
            <div class="flex items-center gap-2">
                <div class="relative">
                    <select id="category-filter" class="appearance-none rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 pr-7 text-[13px] font-semibold text-slate-700 outline-none transition hover:border-slate-300 focus:border-primary-600 focus:ring-1 focus:ring-primary-600 cursor-pointer">
                ...
                <tbody id="customer-table-body" class="divide-y divide-slate-100">
                    @if (count($customers))
                        @foreach($customers as $customer)
                            ...
                            @endphp
                            <tr class="customer-row cursor-pointer transition-colors hover:bg-slate-50/50" data-name="{{ strtolower($customer['name']) }}" data-email="{{ strtolower($customer['email']) }}" data-category="{{ $customer['category'] }}">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-black text-white" style="background-color: {{ $customer['color'] }}">{{ $customer['initials'] }}</div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold {{ $categoryClasses }}">{{ $customer['category'] }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold {{ $statusClasses }}">{{ $customer['status'] }}</span>
                                </td>
                                <td class="px-6 py-4 text-[13px] font-medium text-slate-500">{{ $customer['date'] }}</td>
                                <td class="flex justify-end px-6 py-4 text-right">
                                    <a href="{{ $detailsUrl }}" class="ajax-link inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary-700 text-white shadow-sm transition-all hover:-translate-y-px hover:bg-primary-800 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 cursor-pointer" title="View Customer Details">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
        ...
        <div class="flex flex-col items-center justify-between gap-4 border-t border-slate-100 px-6 py-4 sm:flex-row">
            <div>
                <a href="{{ route('admin.customer-directory') }}" class="ajax-link flex items-center gap-1 text-[13px] font-bold text-primary-800 hover:underline cursor-pointer">
                    View More Records
            ...
            <div class="flex items-center gap-2">
                <button type="button" class="flex h-9 w-9 items-center justify-center rounded border border-slate-200 bg-white text-slate-400 transition hover:bg-slate-50 cursor-pointer">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
                    </svg>
                </button>
                <div class="flex text-[13px] font-semibold">
                    <button type="button" class="flex h-9 w-9 items-center justify-center rounded bg-primary-600 text-white cursor-pointer">1</button>
                    <button type="button" class="flex h-9 w-9 items-center justify-center rounded border border-transparent bg-white text-slate-600 transition hover:border-slate-200 hover:bg-slate-50 cursor-pointer">2</button>
                    <button type="button" class="flex h-9 w-9 items-center justify-center rounded border border-transparent bg-white text-slate-600 transition hover:border-slate-200 hover:bg-slate-50 cursor-pointer">3</button>
                    <span class="flex h-9 w-9 items-center justify-center text-slate-400">...</span>
                    <button type="button" class="flex h-9 w-9 items-center justify-center rounded border border-transparent bg-white text-slate-600 transition hover:border-slate-200 hover:bg-slate-50 cursor-pointer">31</button>
                </div>
                <button type="button" class="flex h-9 w-9 items-center justify-center rounded border border-slate-200 bg-white text-slate-400 transition hover:bg-slate-50 cursor-pointer">
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
    ...
    <div id="verification-modal" class="fixed inset-0 z-[1000] hidden" role="dialog" aria-modal="true" aria-labelledby="verification-modal-title">
        ...
        <div class="relative flex min-h-full items-center justify-center px-4 py-6">
            <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl animate-fade-in">
                <div class="mb-5 flex items-start justify-between gap-4">
                    <div>
                        <h3 id="verification-modal-title" class="text-lg font-extrabold text-slate-900">Approve Customer</h3>
                        <p id="verification-modal-summary" class="mt-1 text-sm text-slate-500">Review this customer application before updating the verification status.</p>
                    </div>
                    <button type="button" onclick="closeVerificationModal()" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 cursor-pointer" aria-label="Close verification modal">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                ...
                <div id="verification-credit-limit-container" class="hidden space-y-4 rounded-2xl border border-primary-100 bg-primary-50/40 p-4">
                    <div>
                        <label for="verification-credit-limit" class="mb-2 block text-[11px] font-bold uppercase tracking-widest text-slate-500">Credit Limit</label>
                        <select id="verification-credit-limit" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 outline-none transition focus:border-primary-600 focus:ring-1 focus:ring-primary-600">
                    ...
                    <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 cursor-pointer">
                        <input id="verification-unlimited-credit" type="checkbox" class="mt-0.5 h-4 w-4 rounded border-slate-300 text-primary-600 focus:ring-primary-600">
                        <div>
                            <span class="block text-sm font-bold text-slate-900">Unlimited Credit Limit</span>
                            <span class="mt-0.5 block text-xs font-medium text-slate-500">When enabled, only the number of days remains required.</span>
                        </div>
                    </label>

                    <div>
                        <label for="verification-credit-days" class="mb-2 block text-[11px] font-bold uppercase tracking-widest text-slate-500">Number Of Days</label>
                        <select id="verification-credit-days" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 outline-none transition focus:border-primary-600 focus:ring-1 focus:ring-primary-600">
                ...
                <div class="mt-6 flex gap-3">
                    <button type="button" onclick="closeVerificationModal()" class="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-50 cursor-pointer">Cancel</button>
                    <button id="verification-confirm-btn" type="button" onclick="confirmVerification()" class="flex-1 rounded-xl bg-primary-600 py-2.5 text-sm font-bold text-white transition hover:bg-primary-700 cursor-pointer shadow-md shadow-primary-600/20">Approve &amp; Save</button>
                </div>
            </div>
        </div>
    </div>
